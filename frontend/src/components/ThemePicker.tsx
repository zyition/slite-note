...
import { themeName, themeSwatch, themeLabel } from "../services/theme";
...
export function ThemePicker({ choice, applied, onSelect, onOpenChange }: ThemePickerProps) {
  ...
  const systemLabel = t.themeSystemLabel(themeName(applied));
  const tooltip = `${t.theme}: ${themeLabel(choice, applied)}`;
